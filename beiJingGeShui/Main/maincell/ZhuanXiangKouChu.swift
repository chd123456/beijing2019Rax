...
import Foundation
import UIKit
class ZhuanXiangKouChu:UITableViewCell {
    let redView = UIView()
    let titleLabel  = UILabel.create(text: "专项扣除项总额（元），没有可不填", textColor: uicolor333, align: .left, fontSize: UIFont.size(size: scale(15), isBody: true))
    ...
    func initWithProperty() {
        ...
        textView.placeholder = "请输入专项扣除金额"
