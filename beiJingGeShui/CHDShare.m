...
+(void)sharedForDescript:(NSString *)descript imageName: (NSString *)imageName setUrl: (NSString *)urlString isUseAirDrop:(BOOL)isUsed currentViewController:(UIViewController *)currentViewController comletedHandle:(void (^)(void))comletedHandle cancelHandle:(void (^)(void))cancelHandle{
    ...
    NSArray *activityItems = @[descript, [UIImage imageNamed:imageName], [NSURL URLWithString:urlString]];
...
+(void)sharedForDescript:(NSString *)descript pdfPath: (NSString *)pdfPath isUseAirDrop:(BOOL)isUsed currentViewController:(UIViewController *)currentViewController comletedHandle:(void (^)(void))comletedHandle cancelHandle:(void (^)(void))cancelHandle{
    NSArray *activityItems = @[descript, [[NSBundle mainBundle] URLForResource:pdfPath withExtension:@"pdf"]];
    UIActivityViewController *activityVC = [[UIActivityViewController alloc] initWithActivityItems:activityItems applicationActivities:nil];
    if (!isUsed) {
        activityVC.excludedActivityTypes = @[UIActivityTypeAirDrop];
    }
